...
# 생성할 필드
- appearance: 보스의 외형 묘사. gender와 description을 근거로 새로 작성 (영문 권장, 1~3문장). 원본에는 외형 데이터가 비어 있으므로 직접 창작.
...
- appearance도 "${theme}" 무대에 실제로 서 있는 모습으로 묘사 (gender·description 기반, 원본에 외형 데이터 없음)
...
export const bossGenTool = [
	{
		...
		function: {
			...
			parameters: {
				type: 'object',
				properties: {
					appearance: { type: 'string', description: 'gender·description 기반 외형 묘사 (영문 권장)' },
				...
				required: ['appearance', 'surface_identity', 'true_identity', 'motivation', 'twist']
			}
		}
	}
];
