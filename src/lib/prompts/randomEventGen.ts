...
choice[1] (확률 업그레이드):
  post 필드를 사용하지 말 것. 대신 postSuccess와 postFail을 각각 작성.

  postSuccess = [높은 등급 보상을 받은 서사적 결과(1~2문장)]
    전체 40~90자. 명확히 긍정적인 감각 표현. 부정 서술 금지. % 숫자 금지.

  postFail = [아무 보상도 받지 못한 결과(1문장)]
...
[출력 예시 (단일 이벤트)]
{
  ...
  "choices": [
    ...
    {
      "text": "속삭임에 귀 기울인다",
      "postSuccess": "귓가에 스친 이름이 선명해지더니 오래된 힘이 온몸에 스며드는 감각이 남았다.",
      "postFail": "기운이 잠시 맴돌다 조용히 사라졌다. 아무것도 남지 않았다."
    }
  ]
}`;
...
이 타입은 narration에서 **힌트만** 제공하고, 긍정처럼 보이지만 위험 요소가 숨어 있음을 암시한다. choice[0]은 서버에서 70% 긍정 / 30% 부정 확률로 결과가 결정되며, 각 결과에 맞는 postSuccess와 postFail을 모두 작성한다.
...
choice[0] (선택한다 — 리스크 감수):
  ...
  postSuccess = [긍정적 결과 서사(1~2문장)]
    전체 40~90자. 보상을 얻거나 좋은 변화가 생긴 감각 표현. 부정 서술 금지. % 숫자 금지.

  postFail = [부정적 결과 서사(1~2문장)]
...
[출력 예시 (단일 이벤트)]
{
  ...
  "choices": [
    {
      "text": "열어본다",
      "postSuccess": "뚜껑을 들추자 달콤한 향과 함께 따뜻한 기운이 스며들었다. 손끝에 무언가 남은 듯한 감각이 오래 머물렀다.",
      "postFail": "향이 훅 퍼지는 순간 어지러움이 밀려왔다. 무언가 빠져나간 듯 몸이 잠시 무거워졌다."
    },
    {
      "text": "지나간다",
...
function eventItemSchema(scenarioType: RandomEventScenarioType) {
	...
	if (scenarioType === 'trade_off') {
		...
		choice1Desc =
			'post 필드 사용 금지. postSuccess: 높은 등급 보상 획득 서사 (40~90자, 긍정만). postFail: 보상 없음 서사 (30~70자, 손해 서술 금지). % 숫자 금지.';
	} else {
		// random_hidden
		choice0Desc =
			'post 필드 사용 금지. postSuccess: 긍정적 결과 서사 (40~90자, 보상/좋은 변화). postFail: 부정적 결과 서사 (40~90자, 손해/불쾌감, 치명적 표현 금지). % 숫자 금지.';
			...
			choices: {
				...
				items: {
					...
					properties: {
						...
						postSuccess: {
							type: 'string',
							description: 'probability_upgrade choice[1] 전용 — 높은 등급 보상 획득 시 서사.'
						},
						postFail: {
							type: 'string',
							description: 'probability_upgrade choice[1] 전용 — 보상 없음 시 서사.'
						}
					},
					required: ['text']
				}
